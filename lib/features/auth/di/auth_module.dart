...
import '../presentation/cubits/forgot_password_cubit/forgot_password_cubit.dart';
import '../presentation/cubits/login_cubit/login_cubit.dart';
import '../presentation/cubits/otp_cubit/otp_cubit.dart';
import '../presentation/cubits/sign_up_cubit/sign_up_cubit.dart';
...
void registerAuthModule() {
  ...
  if (!sl.isRegistered<LoginCubit>()) {
    sl.registerFactory<LoginCubit>(
      () => LoginCubit(sl<Login>(), sl<SocialLogin>(), sl<FailureMessageResolver>()),
    );
  }

  if (!sl.isRegistered<OtpCubit>()) {
    sl.registerFactory<OtpCubit>(
      () => OtpCubit(sl<SendOtp>(), sl<VerifyOtp>(), sl<FailureMessageResolver>()),
    );
  }

  if (!sl.isRegistered<ForgotPasswordCubit>()) {
    sl.registerFactory<ForgotPasswordCubit>(
      () => ForgotPasswordCubit(sl<ResetPassword>(), sl<FailureMessageResolver>()),
    );
  }
}
